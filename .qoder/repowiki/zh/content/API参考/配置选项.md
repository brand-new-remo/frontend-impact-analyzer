...
## 更新摘要
**变更内容**
- 新增 analysis.phasedExecutionThreshold 配置选项的详细说明
- 解释如何通过调整阈值来控制何时启用自动阶段执行
- 说明如何禁用自动阶段执行功能
- 更新配置文件结构与选项章节，添加新的分析配置项
...
**更新** 新增 analysis.phasedExecutionThreshold 配置选项，允许用户控制自动阶段执行的触发条件。
...
**更新** 新增阶段执行控制配置项，允许用户精细控制分析流程的执行方式。
...
## 架构总览
下图展示配置在端到端流程中的作用与流向，包括新增的用户确认步骤和阶段执行控制：
...
- 默认配置键空间：
  ...
  - analysis：上下文预算、是否要求特定文档、聚类深度分析上限、**阶段执行阈值**等
- CLI 覆盖项：
...
- 影响规则与映射：
  - 不同语义标签映射到测试用例模板
- 工作流扩展：
  - 通过 analysis.* 配置控制上下文大小、聚类数量、注释证据上限、**阶段执行阈值**等
