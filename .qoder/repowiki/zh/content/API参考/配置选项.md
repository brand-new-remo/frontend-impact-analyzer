...
- 运行时配置来源：
  ...
  - 命令行参数对配置的临时覆盖
- 依赖工具：
  - uv：推荐使用 uv run --project <skill_root> 执行脚本
  - Python 版本要求：>=3.12
...
## 结论
本配置体系以“默认配置 + 用户配置 + CLI 覆盖”的分层设计实现灵活可控的分析行为。通过合理的上下文预算、噪声过滤与聚类深度控制，既能保证大规模变更的可管理性，又能借助代理工具实现高质量的人机协同深度分析。建议在团队内统一配置与约定，结合 JSON Schema 约束确保输出一致性。
