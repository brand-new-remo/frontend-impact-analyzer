...
## 更新摘要
**变更内容**
- 新增 diff.includePaths 配置选项的详细说明，支持正向路径选择功能
- 更新 diff 配置选项章节，添加新的 includePaths 选项
- 更新配置文件结构与选项章节，添加新的 diff 配置项
- 更新命令行参数章节，添加 --include-path 参数说明
- 更新故障排查指南，添加 includePaths 相关问题排查
...
**更新** 新增 diff.includePaths 配置选项，支持正向路径选择功能，允许用户精确指定哪些目录或文件应包含在diff中，提供更精细的文件选择控制能力。
...
PH["阶段执行阈值<br/>analysis.phasedExecutionThreshold"] --> CLI
CB["批处理大小<br/>analysis.clusterContextBatchSize"] --> CLI
IP["包含路径<br/>diff.includePaths"] --> CLI
```
...
- **阶段执行控制**：新增 `analysis.phasedExecutionThreshold` 配置项，控制自动阶段执行的触发条件
- **批量处理优化**：新增 `analysis.clusterContextBatchSize` 配置项，控制聚类上下文收集的批处理大小
- **正向路径选择**：新增 `diff.includePaths` 配置项，支持精确指定包含在diff中的目录或文件

**更新** 新增正向路径选择配置项，允许用户精确控制diff生成范围，提供更精细的文件选择能力。
...
## 架构总览
下图展示配置在端到端流程中的作用与流向，包括新增的用户确认步骤、阶段执行控制、批量处理优化和正向路径选择：
...
participant PH as "阶段执行阈值"
participant CB as "批处理大小"
participant IP as "包含路径"
...
WF-->>CLI : 返回合并后的配置
CLI->>PH : 读取 analysis.phasedExecutionThreshold
PH-->>CLI : 返回阈值配置
CLI->>CB : 读取 analysis.clusterContextBatchSize
CB-->>CLI : 返回批处理大小配置
CLI->>IP : 读取 diff.includePaths
IP-->>CLI : 返回包含路径配置
...
- 默认配置键空间：
  ...
  - diff：包含路径、忽略目录、忽略文件、忽略通配符
  - analysis：上下文预算、是否要求特定文档、聚类深度分析上限、**阶段执行阈值**、**批处理大小**等
- CLI 覆盖项：
  - --analysis-output-dir：临时覆盖输出目录
  - --project-profile-file：临时覆盖项目档案文件路径
  - --config-file：指定配置文件路径
  - --include-path：临时覆盖包含路径（可重复使用）
- 预检检查：
...
- **正向路径选择控制**：
  - `diff.includePaths`：精确指定包含在diff中的目录或文件列表
  - 支持相对路径（相对于项目根目录），可包含目录和文件
  - 当配置为空时，使用默认的 "."（所有文件）作为包含范围
  - 与忽略规则配合使用，先应用包含规则，再应用排除规则
- **阶段执行阈值控制**：
  - `analysis.phasedExecutionThreshold`：控制自动阶段执行的阈值（默认 1000 行）
  - 当 diff 行数超过阈值时，自动执行 parse 阶段并停止
  - 设置为 0 可完全禁用自动阶段执行功能
- **批量处理优化**：
  - `analysis.clusterContextBatchSize`：控制聚类上下文收集的批处理大小（默认 10）
  - 支持分批处理聚类，减少内存占用
  - 支持独立的批处理模式和合并模式

**更新** 新增正向路径选择配置选项，提供更精细的diff生成控制能力。
...
- [scripts/front_end_impact_analyzer.py:280-308](file://scripts/front_end_impact_analyzer.py#L280-L308)
- [scripts/front_end_impact_analyzer.py:843-844](file://scripts/front_end_impact_analyzer.py#L843-L844)
- [SKILL.md:12-21](file://SKILL.md#L12-L21)
...
- **正向路径选择控制**：
  - `diff.includePaths`：支持精确指定包含在diff中的目录或文件
  - 支持相对路径，可包含目录和文件
  - 当配置为空时，使用默认的 "."（所有文件）作为包含范围
  - 与忽略规则配合使用，先应用包含规则，再应用排除规则
- **阶段执行控制**：
  - `analysis.phasedExecutionThreshold`：默认 1000 行
  - 自动阶段执行：当 diff 行数超过阈值时自动执行 parse 阶段
  - 禁用自动阶段执行：设置阈值为 0
- **批量处理优化**：
  - `analysis.clusterContextBatchSize`：默认 10 个聚类
  - 分批处理上下文收集，减少内存占用
  - 支持独立的批处理模式和合并模式
...
### 配置优先级与继承关系
- 优先级顺序（从高到低）：
  1) 命令行参数覆盖（--analysis-output-dir、--project-profile-file、--config-file、--include-path）
...
- 继承关系：
  ...
  - **正向路径选择**：diff.includePaths 控制diff生成的包含范围
  - **阶段执行阈值**：analysis.phasedExecutionThreshold 控制自动阶段执行行为
  - **批量处理大小**：analysis.clusterContextBatchSize 控制聚类上下文收集的批处理大小
- **用户确认流程**：
...
- 使用流程：
  ...
  - 通过阶段执行机制，先执行 parse 阶段，再执行 scan 和 analyze 阶段
...
- 影响规则与映射：
  - 不同语义标签映射到测试用例模板
- 工作流扩展：
  - 通过 analysis.* 配置控制上下文大小、聚类数量、注释证据上限、**阶段执行阈值**、**批处理大小**等
  - 通过 diff.* 配置排除噪声，提升聚类质量
  - **通过 diff.includePaths 精确控制分析范围**，只分析指定的目录或文件

**更新** 新增正向路径选择对分析行为的影响说明。
...
- **正向路径选择配置**：
  - 默认值：空数组（包含所有文件）
  - 配置示例：`["src/", "common/", "mobile/"]` 或 `["src/pages/HomePage.tsx", "src/components/Button.tsx"]`
  - 精确控制：只分析指定的目录或文件，忽略其他所有文件
  - 与忽略规则配合：先应用包含规则，再应用排除规则
  - 大型项目建议：使用包含路径精确控制分析范围，避免不必要的文件参与分析
- **阶段执行阈值配置**：
  - 默认值：1000 行
  - 调整阈值：在 analysis.phasedExecutionThreshold 中设置期望的行数阈值
  - 禁用自动阶段执行：设置 analysis.phasedExecutionThreshold 为 0
  - 大型项目建议：根据项目规模和资源限制调整阈值，避免单次分析耗时过长
- **批量处理大小配置**：
  - 默认值：10 个聚类
  - 调整批处理大小：在 analysis.clusterContextBatchSize 中设置期望的批处理大小
  - 内存优化：对于大型项目，建议减小批处理大小以减少内存占用
  - 性能平衡：根据硬件资源和项目规模调整批处理大小，在内存使用和处理速度之间找到平衡点
- 最佳实践：
  - 在项目根目录维护 impact-analyzer.config.json，避免每次运行都传参
  - 使用 `diff.includePaths` 精确控制分析范围，只分析相关文件
  ...
  - **正向路径选择优化**：使用包含路径精确控制分析范围，提升分析效率和准确性
  - **阶段执行优化**：根据 diff 规模合理设置阈值，平衡分析质量和执行效率
  - **批量处理优化**：根据内存和 CPU 资源合理设置批处理大小，避免内存溢出

**更新** 新增正向路径选择配置的最佳实践说明。
...
- **正向路径选择依赖**：diff.includePaths 功能依赖于 workflow 模块中的包含路径处理逻辑
- **阶段执行依赖**：自动阶段执行功能依赖于 analysis.phasedExecutionThreshold 配置项
- **批量处理依赖**：聚类上下文收集的批处理功能依赖于 analysis.clusterContextBatchSize 配置项
...
IP["正向路径选择<br/>diff.includePaths"] --> CLI
PH["阶段执行阈值<br/>analysis.phasedExecutionThreshold"] --> CLI
CB["批处理大小<br/>analysis.clusterContextBatchSize"] --> CLI
```
...
- **正向路径选择优化**：通过 diff.includePaths 精确控制分析范围，显著减少分析文件数量
...
- **阶段执行优化**：通过 analysis.phasedExecutionThreshold 合理设置阈值，避免大型 diff 导致的长时间分析阻塞
- **批量处理优化**：通过 analysis.clusterContextBatchSize 合理设置批处理大小，平衡内存使用和处理速度
...
- **正向路径选择问题**：
  - 如果 diff 包含了不需要的文件，检查 diff.includePaths 配置是否正确
  - 确认包含路径相对于项目根目录的相对路径是否正确
  - 验证包含路径是否与忽略规则冲突
  - 使用 --make-diff 重新生成 diff 文件，确认包含路径生效
- **阶段执行问题**：
  - 如果 diff 过大但未触发自动阶段执行，检查 analysis.phasedExecutionThreshold 配置
  - 如果希望禁用自动阶段执行，将 analysis.phasedExecutionThreshold 设置为 0
  - 如果 diff 较小但触发了自动阶段执行，降低 analysis.phasedExecutionThreshold 配置值
- **批量处理问题**：
  - 如果内存不足或处理缓慢，检查 analysis.clusterContextBatchSize 配置
  - 如果希望一次性处理所有聚类，将 analysis.clusterContextBatchSize 设置为较大的值
  - 如果出现内存溢出，减小 analysis.clusterContextBatchSize 配置值

**更新** 新增正向路径选择相关的故障排查指南。
...
**新增的 diff.includePaths 配置选项提供了更精细的文件选择控制能力**。通过精确指定包含在diff中的目录或文件，用户可以显著缩小分析范围，提升分析效率和准确性。这一功能特别适用于大型项目，能够避免不必要的文件参与分析，减少内存占用和处理时间。

**新增的 analysis.clusterContextBatchSize 配置选项提供了更精细的性能控制能力**。通过调整批处理大小，用户可以根据硬件资源和项目规模，灵活控制聚类上下文收集的内存使用和处理速度。默认 10 个聚类的批处理大小适用于大多数场景，但大型项目可能需要更小的批处理大小以避免内存溢出，或者更大的批处理大小以提高处理效率。

**新增的 analysis.phasedExecutionThreshold 配置选项提供了更精细的分析流程控制能力**。通过调整阈值，用户可以根据项目规模、资源限制和分析需求，灵活控制何时启用自动阶段执行。默认 1000 行的阈值适用于大多数场景，但大型项目可能需要更高的阈值或完全禁用自动阶段执行功能。

**配置安全增强机制确保了分析流程的安全性和可控性**。通过 `userActionRequired` 字段和强制用户确认步骤，系统能够在配置初始化时阻止自动继续执行，要求用户必须审查并确认配置文件的正确性。这一机制有效防止了用户误用默认配置而产生意外的分析结果，特别是在 diff 忽略规则和路径配置等关键设置上。
...
- paths
  ...
  - outputDir：运行产物输出目录
- diff
  - **includePaths**：**包含在diff中的目录/文件列表（空 = 所有文件）**
  - ignoreDirs：忽略目录列表
  - ignoreFiles：忽略文件列表
  - ignoreGlobs：忽略通配符列表
- analysis
  ...
  - **phasedExecutionThreshold**：**自动阶段执行阈值（默认 1000 行）**
  - **clusterContextBatchSize**：**聚类上下文批处理大小（默认 10 个聚类）**
...
### 正向路径选择配置详解
- **配置键**：diff.includePaths
- **默认值**：空数组（表示包含所有文件）
- **功能描述**：精确指定包含在diff中的目录或文件列表
- **工作原理**：
  - 支持相对路径（相对于项目根目录）
  - 可包含目录和文件，支持目录末尾保留斜杠
  - 当配置为空时，使用默认的 "."（所有文件）作为包含范围
...
- **使用场景**：
  ...
  - 针对特定文件类型进行专门分析
- **配置方法**：
  - 在 impact-analyzer.config.json 中设置 diff.includePaths
  - 支持数组格式，每个元素为相对路径字符串
  - 可与 --include-path 命令行参数配合使用
- **最佳实践**：
  - 小型项目：保持默认空数组，分析所有文件
  - 中型项目：使用包含路径精确控制分析范围
  - 大型项目：使用包含路径只分析相关模块或组件
  - 与忽略规则配合：先包含必要文件，再排除不需要的文件

**新增** 新增正向路径选择配置的详细说明。

**章节来源**
- [scripts/analyzer/workflow.py:31-32](file://scripts/analyzer/workflow.py#L31-L32)
- [scripts/analyzer/workflow.py:433-450](file://scripts/analyzer/workflow.py#L433-L450)
- [scripts/front_end_impact_analyzer.py:843-844](file://scripts/front_end_impact_analyzer.py#L843-L844)

### 阶段执行阈值配置详解
- **配置键**：analysis.phasedExecutionThreshold
- **默认值**：1000 行
- **功能描述**：控制自动阶段执行的触发条件
- **工作原理**：
  - 当 diff 文件的行数超过配置的阈值时，自动执行 parse 阶段并停止
  - 用户随后可以分别执行 scan 和 analyze 阶段
- **使用场景**：
  - 大型 diff 分析：避免单次分析耗时过长
  - 资源受限环境：通过分阶段执行优化资源使用
  - 逐步调试：分阶段检查分析过程中的中间结果
- **配置方法**：
  - 在 impact-analyzer.config.json 中设置 analysis.phasedExecutionThreshold
  - 支持任意正整数配置
  - 设置为 0 可完全禁用自动阶段执行功能
- **最佳实践**：
  - 小型项目：保持默认值 1000 行
  - 中型项目：根据平均 diff 规模调整至 500-2000 行
  - 大型项目：根据资源情况调整至 2000-10000 行
  - 资源紧张环境：适当降低阈值以启用分阶段执行

**新增** 新增阶段执行阈值配置的详细说明。

**章节来源**
- [scripts/front_end_impact_analyzer.py:595-601](file://scripts/front_end_impact_analyzer.py#L595-L601)
- [scripts/analyzer/workflow.py:61-62](file://scripts/analyzer/workflow.py#L61-L62)
- [SKILL.md:97-98](file://SKILL.md#L97-L98)
- [references/agent-usage.md:30-33](file://references/agent-usage.md#L30-L33)

### 批处理大小配置详解
- **配置键**：analysis.clusterContextBatchSize
- **默认值**：10 个聚类
- **功能描述**：控制聚类上下文收集的批处理大小
- **工作原理**：
  - 支持分批处理聚类上下文，减少内存占用
  - 支持独立的批处理模式和合并模式
  - 每批处理指定数量的聚类，避免一次性处理过多聚类
- **使用场景**：
  - 大型项目聚类：避免内存溢出
  - 资源受限环境：优化内存使用
  - 逐步处理：分阶段处理聚类上下文
- **配置方法**：
  - 在 impact-analyzer.config.json 中设置 analysis.clusterContextBatchSize
  - 支持任意正整数配置
  - 设置为较大的值可一次性处理所有聚类
- **最佳实践**：
  - 小型项目：保持默认值 10 个聚类
  - 中型项目：根据内存情况调整至 5-20 个聚类
  - 大型项目：根据资源情况调整至 1-10 个聚类
  - 内存紧张环境：适当降低批处理大小以避免内存溢出

**新增** 新增批量处理大小配置的详细说明。

**章节来源**
- [scripts/front_end_impact_analyzer.py:698-700](file://scripts/front_end_impact_analyzer.py#L698-L700)
- [scripts/analyzer/workflow.py:62-63](file://scripts/analyzer/workflow.py#L62-L63)
- [references/agent-usage.md:30-33](file://references/agent-usage.md#L30-L33)